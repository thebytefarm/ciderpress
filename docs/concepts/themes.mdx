...
A theme in ciderpress is a **brand identity** with one or more **variants**. Each variant is a complete set of design tokens that targets either the `dark` or `light` aesthetic. ciderpress ships with four built-in themes — Honeycrisp, Granny Smith, Midnight, and Arcade — and dark is the framework's baseline (the initial paint is always dark unless the user explicitly toggles to light on a multi-variant theme).
...
The `name` accepts any built-in theme (`honeycrisp`, `grannysmith`, `midnight`, `arcade`) or any custom theme registered in the top-level `themes` array. The legacy slug `'default'` is preserved as an alias for `'honeycrisp'`. An unknown name writes a build-time warning to stderr and falls back to `honeycrisp`.

### Options

| Property     | Type                                                      | Default         | Description                                                                                                |
| ------------ | --------------------------------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------- |
| `name`       | `'honeycrisp' \| 'grannysmith' \| 'midnight' \| 'arcade'` | `'honeycrisp'`  | Built-in or registered theme to use. `'default'` is accepted as an alias for `'honeycrisp'`                |
