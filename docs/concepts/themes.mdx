...
A theme in ciderpress is a **brand identity** with one or more **variants**. Each variant is a complete set of design tokens that targets either the `dark` or `light` aesthetic. ciderpress ships with five built-in themes — Honeycrisp, Granny Smith, Mulled, Midnight, and Arcade — and dark is the framework's baseline (the initial paint is always dark unless the user explicitly toggles to light on a multi-variant theme).
...
Deep cider burgundy — the "evening / premium" branch of the apple family. Primary `#991b1b` with both `dark` and `light` variants. The light variant pairs warm cream surfaces (`#fbf6f4`) with deep burgundy ink for a parchment read; the dark variant uses the same near-black canvas as Honeycrisp / Granny Smith so the apple themes stay visually consistent at the chrome level on dark.
...
The `name` accepts any built-in theme (`honeycrisp`, `grannysmith`, `mulled`, `midnight`, `arcade`) or any custom theme registered in the top-level `themes` array. The legacy slug `'default'` is preserved as an alias for `'honeycrisp'`. An unknown name writes a build-time warning to stderr and falls back to `honeycrisp`.

### Options

| Property     | Type                                                      | Default         | Description                                                                                                |
| ------------ | --------------------------------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------- |
| `name`       | `'honeycrisp' \| 'grannysmith' \| 'mulled' \| 'midnight' \| 'arcade'` | `'honeycrisp'`  | Built-in or registered theme to use. `'default'` is accepted as an alias for `'honeycrisp'`                |
...
| ------------ | --------------------------------- | ----------- | --------------- |
| Honeycrisp   | General-purpose docs (canonical)  | dark, light | `dark`          |
| Granny Smith | Green-branded, accessible docs    | dark, light | `dark`          |
| Mulled       | Premium / evening reads, warm-tone | dark, light | `dark`          |
